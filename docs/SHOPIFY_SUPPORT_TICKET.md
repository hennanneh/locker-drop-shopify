...
- ✅ **App is installed** (uninstalled/reinstalled once); carrier-service shipping rates from the same app work correctly (the "LockerDrop @ …" shipping option appears and is selectable at checkout). The same app's `admin.order-details.block.render` extension **does render** in the Shopify admin — only the checkout/customer-account surfaces are blank.
